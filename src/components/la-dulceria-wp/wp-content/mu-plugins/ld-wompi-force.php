...
add_action('woocommerce_api_wompi_webhook', function () {
    ...
    if (!empty($evt_secret) && !empty($checksum)) {
        ...
        if (!hash_equals($firma_esperada, $checksum)) {
            file_put_contents($log_file, date('H:i:s') . ' MU-FIRMA INVALIDA esperada=' . $firma_esperada . ' recibida=' . $checksum . " — procesando igual (secret pendiente de corrección)\n", FILE_APPEND);
            // TEMPORAL: continuar aunque la firma no coincida hasta corregir el secret
        }
    } else {

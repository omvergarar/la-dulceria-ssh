...
<div style="padding:48px 24px;min-height:80vh;background:var(--bg-soft);">
  <div class="ld-container">
    ...
    <div class="ld-catalogo-header">
      <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;">
        ...
        <?php foreach ($categorias as $cat): ?>
          <a href="<?= esc_url(add_query_arg('cat', $cat->slug, home_url('/tienda'))) ?>"
             class="ld-filtro-btn <?= $cat_actual === $cat->slug ? 'active' : '' ?>">
            <?= esc_html($cat->name) ?>
          </a>
        <?php endforeach; ?>
      </div>
      <form method="get" action="<?= esc_url(home_url('/tienda')) ?>" style="display:flex;gap:8px;">
        <?php if ($cat_actual): ?>
    ...
    <p style="color:var(--text-medium);font-size:.875rem;margin-bottom:20px;">
    ...
    <div style="text-align:center;padding:80px 20px;">
      ...
      <a href="<?= home_url('/tienda') ?>" class="btn-primary">Ver todos los productos</a>
    </div>
    <?php endif; ?>
  </div>
</div>
